...
short setGlobalPath(const char *outPathStr)
{
	...
	if(strlen(outPathStr)!=0)
		strcpy(outputPathStr, outPathStr);
	else
		strcpy(outputPathStr, OUTPUT_DIR_DEFAULT);
...
short generateConfigFile(const string &configFile){
	...
	if(!outfile.is_open()){
		cerr << "Cannot open file '" << configFile << "', error!" << endl;
		return FAILED;
	}

	outfile << "# This is a configuration example file." << endl << endl;

	outfile << "QUERY" << endl;
	outfile << "# Assembly result file, e.g. scaffolds or contigs in fasta format." << endl;
	outfile << "/home/user/misass/misFinder/spombe/scaffolds.fasta" << endl;
	outfile << "END" << endl << endl;

	outfile << "REF" << endl;
	outfile << "# Reference segments in fasta format." << endl;
	outfile << "/home/user/misass/misFinder/spombe/refs/GCF_000002945.1_ASM294v2_genomic.fa" << endl;
	outfile << "#/home/user/misass/misFinder/spombe/refs/NC_003424.3.fa" << endl;
	outfile << "#/home/user/misass/misFinder/spombe/refs/NC_003423.3.fa" << endl;
	outfile << "#/home/user/misass/misFinder/spombe/refs/NC_003421.2.fa" << endl;
