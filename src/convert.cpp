...
short getItemNums(int64_t *itemNumSubjectArray, int64_t *itemNumQueryArray, int64_t *itemNumTmpQueryArray, int64_t *itemNumMatchItemArray, char *parseResultFile)
{
	...
	if(getQuerySubjectNumAndMatchItemNum(itemNumQueryArray, itemNumTmpQueryArray, itemNumMatchItemArray, parseResultFile)==FAILED)
	{
...
{
	int32_t len, fileStatus;
	char line[LINE_CHAR_MAX+1], *pch;
	FILE *fpParseResult;
	set<string> subjectname_set;
	string subjectname, line_str;
...
//	}
	...
	*itemNumSubjectArray = 0;
	while(1)
	{
		...
		if(*pch == '>')
		{
			// get query item count
			line_str = line + 1;
			line_vec = split(line_str, "\t");
			subjectname = line_vec.at(2);
			if(subjectname_set.find(subjectname)==subjectname_set.end()) subjectname_set.insert(subjectname);
...
//			}
	...
	*itemNumSubjectArray = subjectname_set.size();
...
short getQuerySubjectNumAndMatchItemNum(int64_t *itemNumQueryArray, int64_t *tmpQuerySubjectNum, int64_t *itemNumMatchItemArray, char *parseResultFile)
{
	...
	FILE *fpParseResult;
	set<string> queryname_set;
	string queryname, line_str;
	vector<string> line_vec;
	...
	*itemNumMatchItemArray = 0;
	while(1)
	{
		...
		if(line[0]!='>')
			...
			line_str = line + 1;
			line_vec = split(line_str, "\t");
			queryname = line_vec.at(0);
			if(queryname_set.find(queryname)==queryname_set.end()) queryname_set.insert(queryname);
		}
	}

	*itemNumQueryArray = queryname_set.size();
...
short fillSubjectArray(subject_t *subjectArray, int64_t itemNumSubjectArray, char *parseResultFile)
{
	...
	char line[LINE_CHAR_MAX+1], *pch;
	FILE *fpParseResult;
	vector<string> subjectname_vec;
	vector<int32_t> subjectlen_vec;
	string subjectname, line_str;
	vector<string> line_vec;
	...
	while(1)
	{
		...
		if(*pch == '>')
		{
			...
			line_vec = split(line_str, "\t");
			subjectname = line_vec.at(2);
			if(find(subjectname_vec.begin(), subjectname_vec.end(), subjectname) == subjectname_vec.end()){
				subjectname_vec.push_back(subjectname);
				subjectlen_vec.push_back(stoi(line_vec.at(3)));
			}
...
//				break;
	...
	// fill data
	if(subjectname_vec.size()>0){
		for(size_t i=0; i<subjectname_vec.size(); i++){
			// subjectTitle
			subjectArray[i].subjectID = i + 1;
			subjectArray[i].subjectTitle = (char*) calloc (subjectname_vec.at(i).size()+1, sizeof(char));
			strcpy(subjectArray[i].subjectTitle, subjectname_vec.at(i).c_str());
			subjectArray[i].subjectTitleLen = subjectname_vec.at(i).size();
			// subjectLen
			subjectArray[i].subjectLen = subjectlen_vec.at(i);
			subjectArray[i].circularFlag = YES;
		}
	}else{
		printf("line=%d, In %s(), there is no subjects, error!\n", __LINE__, __func__);
		return FAILED;
	}
...
short fillSingleTmpQueryMatchInfo(tmpQuery_t *pTmpQuery, matchItem_t *pMatchItemArray, int64_t itemNumMatchItemArray, int64_t tmpQueryID, subject_t *subjectArray, int64_t itemNumSubjectArray, FILE *fpParsedResult)
{
	...
	char line[LINE_CHAR_MAX+1];
	int32_t len;
	int64_t i, matchItemNum, subjectID;
	...
	if(ch=='>')//start of a new contig
	{
		...
		// query title
		pTmpQuery->queryTitle = (char *) calloc (line_vec.at(0).size()+1, sizeof(char));
		if(pTmpQuery->queryTitle==NULL)
		{
			printf("line=%d, In %s(), cannot allocate memory, error!\n", __LINE__, __func__);
			return ERROR;
		}
		strcpy(pTmpQuery->queryTitle, line_vec.at(0).c_str());

		// query length
		pTmpQuery->queryLen = stoi(line_vec.at(1));

		// subject title
		subjectID = -1;
		for(i=0; i<itemNumSubjectArray; i++){
			if(strcmp(line_vec.at(2).c_str(), subjectArray[i].subjectTitle)==0){
				subjectID = i + 1;
				break;
			}
		}
		...
		{
			printf("line=%d, In %s(), incorrect subjectTitle, itemNumSubjectArray=%ld, error!\n", __LINE__, __func__, itemNumSubjectArray);
			return ERROR;
		}

		// subject length
		subjectArray[subjectID-1].subjectLen = stoi(line_vec.at(3));
	}
...
short convertQueryMatchInfo(queryMatchInfo_t *queryMatchInfoSet, tmpQuery_t *tmpQueryArray, int64_t itemNumTmpQueryArray)
{
	...
	queryID = 0;
	tmpQuerySubjectNum = 0;
	for(i=0; i<itemNumTmpQueryArray; i+=tmpQuerySubjectNum)
	{
		...
		// get the subjectNum
		tmpQuerySubjectNum = 0;
		for(j=i; j<itemNumTmpQueryArray; j++)
		{
			if(strcmp(tmpQueryArray[j].queryTitle, tmpQueryArray[i].queryTitle)==0 and tmpQueryArray[j].matchItemNum>0)
				tmpQuerySubjectNum ++;
		}
		...
		tmpSubjectRow = 0;
		for(j=0; j<tmpQuerySubjectNum; j++){
...
short saveQueryMatchInfoToFile(char *queryMatchInfoFile, queryMatchInfo_t *queryMatchInfoSet)
{
	...
	if(fwrite(queryMatchInfoSet->subjectArray, sizeof(subject_t), queryMatchInfoSet->itemNumSubjectArray, fpQueryMatchInfo) != (size_t)queryMatchInfoSet->itemNumSubjectArray)
	{
		printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
		return FAILED;
	}
	for(i=0; i < queryMatchInfoSet->itemNumSubjectArray; i++)
	{
		if(fwrite(queryMatchInfoSet->subjectArray[i].subjectTitle, sizeof(char), queryMatchInfoSet->subjectArray[i].subjectTitleLen+1, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->subjectArray[i].subjectTitleLen+1)
		{
			printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
			return FAILED;
		}
	...
	if(fwrite(queryMatchInfoSet->queryArray, sizeof(query_t), queryMatchInfoSet->itemNumQueryArray, fpQueryMatchInfo) != (size_t)queryMatchInfoSet->itemNumQueryArray)
	{
		printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
		return FAILED;
	}
	for(i=0; i<queryMatchInfoSet->itemNumQueryArray; i++)
	{
		if(fwrite(queryMatchInfoSet->queryArray[i].queryTitle, sizeof(char), queryMatchInfoSet->queryArray[i].queryTitleLen+1, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->queryArray[i].queryTitleLen+1)
		{
			printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
			return FAILED;
		}
		...
		if(queryMatchInfoSet->queryArray[i].querySubjectNum>0)
		{
			...
			if(fwrite(queryMatchInfoSet->queryArray[i].querySubArray, sizeof(querySubject_t), queryMatchInfoSet->queryArray[i].querySubjectNum, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->queryArray[i].querySubjectNum)
			{
				printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
				return FAILED;
			}
			...
			for(j=0; j<queryMatchInfoSet->queryArray[i].querySubjectNum; j++)
			{
				...
				{
					if(fwrite(queryMatchInfoSet->queryArray[i].querySubArray[j].validSegArray, sizeof(validSegment_t), queryMatchInfoSet->queryArray[i].querySubArray[j].validSegmentNum, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->queryArray[i].querySubArray[j].validSegmentNum)
					{
						printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
						return FAILED;
					}
		...
		if(queryMatchInfoSet->queryArray[i].queryReadSetNum>0)
		{
			if(fwrite(queryMatchInfoSet->queryArray[i].queryReadSetArray, sizeof(queryReadSet_t), queryMatchInfoSet->queryArray[i].queryReadSetNum, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->queryArray[i].queryReadSetNum)
			{
				printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
				return FAILED;
			}
			...
			for(j=0; j<queryMatchInfoSet->queryArray[i].queryReadSetNum; j++)
			{
				if(queryMatchInfoSet->queryArray[i].queryReadSetArray[j].queryReadNum>0)
				{
					if(fwrite(queryMatchInfoSet->queryArray[i].queryReadSetArray[j].queryReadArray, sizeof(queryRead_t), queryMatchInfoSet->queryArray[i].queryReadSetArray[j].queryReadNum, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->queryArray[i].queryReadSetArray[j].queryReadNum)
					{
						printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
						return FAILED;
					}
		...
		if(queryMatchInfoSet->queryArray[i].globalValidSegNum>0)
		{
			if(fwrite(queryMatchInfoSet->queryArray[i].globalValidSegArray, sizeof(globalValidSeg_t), queryMatchInfoSet->queryArray[i].globalValidSegNum, fpQueryMatchInfo)!=(size_t)queryMatchInfoSet->queryArray[i].globalValidSegNum)
			{
				printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
				return FAILED;
			}
	...
	if(fwrite(queryMatchInfoSet->matchItemArray, sizeof(matchItem_t), queryMatchInfoSet->itemNumMatchItemArray, fpQueryMatchInfo) != (size_t)queryMatchInfoSet->itemNumMatchItemArray)
	{
		printf("line=%d, In %s(), cannot write information to  binary file, error!\n", __LINE__, __func__);
		return FAILED;
	}
...
short loadQueryMatchInfoFromFile(queryMatchInfo_t **queryMatchInfoSet, char *queryMatchInfoFile)
{
	...
	*queryMatchInfoSet = (queryMatchInfo_t *) calloc (1, sizeof(queryMatchInfo_t));
	...
	if(fread((*queryMatchInfoSet)->subjectArray, sizeof(subject_t), (*queryMatchInfoSet)->itemNumSubjectArray, fpQueryMatchInfo) != (size_t)(*queryMatchInfoSet)->itemNumSubjectArray)
	{
		printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
		return FAILED;
	}
	for(i=0; i < (*queryMatchInfoSet)->itemNumSubjectArray; i++)
	{
		...
		if(fread((*queryMatchInfoSet)->subjectArray[i].subjectTitle, sizeof(char), (*queryMatchInfoSet)->subjectArray[i].subjectTitleLen+1, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->subjectArray[i].subjectTitleLen+1)
		{
			printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
			return FAILED;
		}
	...
	if(fread((*queryMatchInfoSet)->queryArray, sizeof(query_t), (*queryMatchInfoSet)->itemNumQueryArray, fpQueryMatchInfo) != (size_t)(*queryMatchInfoSet)->itemNumQueryArray)
	{
		printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
		return FAILED;
	}
	for(i=0; i<(*queryMatchInfoSet)->itemNumQueryArray; i++)
	{
		...
		// fill queryTitle
		if(fread((*queryMatchInfoSet)->queryArray[i].queryTitle, sizeof(char), (*queryMatchInfoSet)->queryArray[i].queryTitleLen+1, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->queryArray[i].queryTitleLen+1)
		{
			printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
			return FAILED;
		}
		...
		if((*queryMatchInfoSet)->queryArray[i].querySubjectNum>0)
		{
			...
			if(fread((*queryMatchInfoSet)->queryArray[i].querySubArray, sizeof(querySubject_t), (*queryMatchInfoSet)->queryArray[i].querySubjectNum, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->queryArray[i].querySubjectNum)
			{
				printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
				return FAILED;
			}
			...
			for(j=0; j<(*queryMatchInfoSet)->queryArray[i].querySubjectNum; j++)
			{
				if((*queryMatchInfoSet)->queryArray[i].querySubArray[j].validSegmentNum>0)
				{
					...
					if(fread((*queryMatchInfoSet)->queryArray[i].querySubArray[j].validSegArray, sizeof(validSegment_t), (*queryMatchInfoSet)->queryArray[i].querySubArray[j].validSegmentNum, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->queryArray[i].querySubArray[j].validSegmentNum)
					{
		...
		if((*queryMatchInfoSet)->queryArray[i].queryReadSetNum>0)
		{
			...
			if(fread((*queryMatchInfoSet)->queryArray[i].queryReadSetArray, sizeof(queryReadSet_t), (*queryMatchInfoSet)->queryArray[i].queryReadSetNum, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->queryArray[i].queryReadSetNum)
			{
				printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
				return FAILED;
			}
			...
			for(j=0; j<(*queryMatchInfoSet)->queryArray[i].queryReadSetNum; j++)
			{
				if((*queryMatchInfoSet)->queryArray[i].queryReadSetArray[j].queryReadNum>0)
				{
					...
					if(fread((*queryMatchInfoSet)->queryArray[i].queryReadSetArray[j].queryReadArray, sizeof(queryRead_t), (*queryMatchInfoSet)->queryArray[i].queryReadSetArray[j].queryReadNum, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->queryArray[i].queryReadSetArray[j].queryReadNum)
					{
						printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
						return FAILED;
					}
		...
		if((*queryMatchInfoSet)->queryArray[i].globalValidSegNum>0)
		{
			...
			if(fread((*queryMatchInfoSet)->queryArray[i].globalValidSegArray, sizeof(globalValidSeg_t), (*queryMatchInfoSet)->queryArray[i].globalValidSegNum, fpQueryMatchInfo)!=(size_t)(*queryMatchInfoSet)->queryArray[i].globalValidSegNum)
			{
				printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
				return FAILED;
			}
	...
	if(fread((*queryMatchInfoSet)->matchItemArray, sizeof(matchItem_t), (*queryMatchInfoSet)->itemNumMatchItemArray, fpQueryMatchInfo) != (size_t)(*queryMatchInfoSet)->itemNumMatchItemArray)
	{
		printf("line=%d, In %s(), cannot read information to  binary file, error!\n", __LINE__, __func__);
		return FAILED;
	}
