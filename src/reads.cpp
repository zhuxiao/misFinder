...
short constructReadsetByPEFastaInterleaved(readSet_t *readSet, char **readsFileNames, int32_t readsFileNum)
{
	readBuf_t *readBuf;
	FILE* srcfp;
	int32_t i, tmpFileID, tmpReadsNum, seqIntEntriesNum;
	//char seq_data[MAX_READ_LEN_IN_BUF+1]; // read sequence, read quality data which is encoded in ASCII
	int64_t tmpReadCount;
...
short addReadToReadset(readBuf_t *pBufRead, readSet_t *readSet)
{
	...
	if(pBufRead->validFlag==YES)
	{
		...
		if(pReadseqBlockTmp->rowsNum + entriesNum > (uint32_t)readSet->maxEntryNumReadseqBlock){
		...
		{ // add the new readseq item into readseq blocks and readseq hash item blocks
			...
			if(pReadseqHashItemBlockTmp->itemNum >= (uint32_t)readSet->maxItemNumPerReadseqHashItemBlock){
			...
			// set the valid pointer
			if(pReadseqBlockTmp->rowsNum >= (uint32_t)readSet->maxEntryNumReadseqBlock){
				// add new readseq block
				if(addNewBlockReadseq(readSet)==FAILED)
				{
					printf("line=%d, In %s(), cannot add new readseq block, Error!\n", __LINE__, __func__);
	...
	if(pReadBlockTmp->itemNum >= (uint32_t)readSet->maxItemNumPerReadBlock){
		// add new read block
