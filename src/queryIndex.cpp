...
{
	int32_t i, queryLen, startBasePos, endBasePos;
	int32_t kmerSize, hashTableSize, entriesPerKmer, lastEntryBaseNum;
	char *querySeq;
	uint64_t *pKmerSeqIntDoing, hashcode;
...
{
	int32_t i, baseNum, startRow, endRow, validStartRow, validBaseFlag;
...
short countQueryKmer(uint64_t hashcode, uint64_t *kmerSeqInt, queryIndex_t *queryIndex)
{
	...
	if(!kmer)
	{
		...
		if(pKmerBlock->itemNum >= (uint32_t)queryIndex->maxItemNumPerKmerBlock)
		{
			// add new kmer block
			if(addNewBlockQueryKmer(queryIndex)==FAILED)
			{
...
{
	int32_t i, queryID, queryLen, startBasePos, queryPos, endBasePos;
	int32_t kmerSize, hashTableSize, entriesPerKmer, lastEntryBaseNum;
	char *querySeq;
	uint64_t pKmerSeqInt[queryIndex->entriesPerKmer], hashcode;
	query_t *queryArray;
